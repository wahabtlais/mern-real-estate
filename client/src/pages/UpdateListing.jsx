...
const UpdateListing = () => {
	...
	useEffect(() => {
		const fetchListing = async () => {
			...
			if (!res.ok) throw new Error("Could not retrieve listing");
	...
	const handleImageSubmit = () => {
		if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
			...
			setImageUploadError("You can only add up to 6 images only.");
			setUploading(false);
		}
	};
	const storeImage = async (file) => {
		// TODO: Store the image in a database and return its URL here
						...
						formData.imageUrls.map((url, index) => (
							...
							>
								...
								<button
									...
									className="text-red-600 uppercase hover:opacity-85"
								>
									Delete
								</button>
							</div>
